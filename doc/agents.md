...
- 数据库已新增 `agent_runs`、`agent_events`、`agent_tool_calls`，用于记录 run lifecycle、tool call lifecycle 和 harness inspection；`agent_tool_calls` 对内按 run scope 存储 provider tool call id，对外仍保持 UI / approval 使用的 tool call id；`agents.inspectRun` RPC 已能按 `runId + sessionId` 返回 run / events / toolCalls 只读 trace，renderer tool trace 会从 delegation tool output 的 `subRunId` 懒加载 child run trace，`agent-run-trace.ts` 已能把 inspected traces 投影成稳定 run graph preview nodes / edges 和紧凑 display rows，child trace 面板在有 `parentRunId` 时会加载父 trace 并展示父子 run graph preview；完整 workbench 级 run graph UI 仍保留在后续阶段。
- 首版工具包含只读的 `searchFiles`（`rg --json` 内容搜索）、`findFiles`（路径名查找）、`fileInfo`（结构化文件元数据）、`listDirectory`、`readFile`、`listProjectTree`、`gitDiff`、`memorySearch`（受 `settings.memory.enabled` / `autoRetrieve` 控制，并复用现有 memory scope / project 过滤），写入/检查/网络类 `applyPatch`、`editFile`、`writeFile`、`runCheck`、`rtkCommand`、`webSearch` 进入权限判断；file-like tools 已补 secret-like path 与 symlink 边界保护，`fileInfo` / `listDirectory` / `readFile` / `editFile` / `writeFile` 已消费 `ExecutionEnv.fileSystem` Result 边界并对目标 symlink no-follow，`searchFiles` / `applyPatch` / `runCheck` / `rtkCommand` 已消费 `ExecutionEnv.shell.exec()` typed Result，`webSearch` 通过 DuckDuckGo Instant Answer API 返回 bounded 结果且必须 approval，`applyPatch` 会在执行前解析 patch header 并拒绝 secret-like target path；`harness-operator` 可用 `agentEventsSearch` 与 `agentRunInspect` 做只读诊断，且 inspect 前会校验 run 仍属于当前 chat session / project。内建工具已补 capability manifest（`read-fs` / `write-fs` / `shell` / `network` / `git` / `memory` / `agent-run`），`tool-policy.ts` 已提供最小 policy compiler，用 manifest risk 和显式 skill capability 编译可见工具集合；permission engine 已用 manifest capability 做第一层读 / 写 / network / shell 分流，`runCheck` 会自动允许 `vp check` / `vp test run` / package-level `vp run ...` 这类 bounded verification command，为后续 MCP / skill extension policy compiler 铺底。
...
- `apps/desktop/src/main/agents/agent-session-tree.ts` 已提供 in-memory append-only session tree：message / leaf / branch summary / compaction summary / custom message entry、leaf move、branch summary 注入、compaction context reset、自定义消息排除出 model context，并会拒绝移动到未知 entry；`agent-session-events.ts` 已把 prepared model messages、branch move、compaction summary、custom message、queued steering / follow-up custom entry 和 provider finish 后的 assistant / tool response messages 都以 `agent_session_entry_appended` 持久化到 event store，支持从 event log 重建一轮完整 model context，并能列出尚未 replay 到 user model message 的 pending queue；同时已提供 `createAgentSessionQueuedMessageWriter()`，可直接把 stateful `Agent` 的 queued message write callback 接到同一 event payload；`streamAgentChat()` 在 approval resume 时会用已持久化 context 重建 provider messages、从 event log replay pending queue，并只追加缺失 session entry；非 resume 的下一次 agent 请求也会 drain 最新 completed top-level run 的 pending queued messages，避免 UI 在上轮运行中排入的 follow-up 永远停留在 event log；这些行为已从旧 `streamText` mock 迁到真实 runtime harness 覆盖。当前 chat route 已在 Agents 开启且请求进行中时通过 composer 调用 `agents.queueMessage` 写入 queued steering / follow-up；尚未覆盖完整 branchable session 持久化、自动触发 follow-up 和 chat session 生命周期。
- `apps/desktop/src/main/agents/agent.ts` 已提供正式 stateful `Agent` 外壳：持有 model / tools / active tools / messages / resources / system prompt / thinking level，支持 `prompt()`、`continue()`、mutators、busy guard、`abort()`、`waitForIdle()`、`steer()` / `followUp()` 内存队列、初始 queued message replay seed、`"all"` / `"one-at-a-time"` drain 模式、queued message 写入 callback 和 subscriber snapshot；每个 provider turn 会隔离入口快照，运行中 mutator 可通过 `prepareNextTurn` 刷新同一 loop 的下一轮 model / resources / system prompt / thinking level / active tools / tools；subscriber 异常不会打断 turn；尚未接入现有 chat session 生命周期或 event store 持久化。
...
- `apps/desktop/src/main/agents/agent-turn-state.ts` 已提供每轮请求的只读快照：messages / tools / stream options 浅拷贝冻结、system prompt provider 每轮只解析一次，provider credentials resolver 保留为请求期重取；`streamAgentChat()` 主 provider 请求已通过该快照生成 system / messages / headers / metadata，尚未接入 `agent-loop.ts` 与 settings setter。
- `apps/desktop/src/main/agents/agent-errors.ts` 已提供共享 `AgentRuntimeError`：包含高层 runtime error code 与 `cause` 保留；`agent-state.ts` 的 busy guard 与 provider stream hook 已复用该错误类型，主 provider request 准备阶段的 hook 失败会把 run 标记为 failed；主 provider stream 创建失败会包装为 `AgentRuntimeError("provider")` 并保留 cause；其他 runtime / session / tool 错误尚未统一迁移。
...
6. pending session writes（queue 中的 steer / follow-up 消息）已有 `agent_events` custom entry helper、`createAgentSessionQueuedMessageWriter()` 适配层、pending replay 计算和下一次请求 drain 最新 completed run queue 的 runtime 接线；当前 chat route 已能通过 composer 写入 queued steering / follow-up，后续接入自动触发 follow-up 与 self-managed loop 生命周期。
...
| ----------------------------- | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AgentHarness`                | `packages/agent/src/harness/agent-harness.ts`                                       | [`agent-runtime.ts`](../apps/desktop/src/main/agents/agent-runtime.ts) 中 `streamAgentChat()` + [`agent.ts`](../apps/desktop/src/main/agents/agent.ts) + [`agent-state.ts`](../apps/desktop/src/main/agents/agent-state.ts)                                                                                             | **部分**：创建 `agent_run`、写 events、子 agent 委派；已补 stateful `Agent` 外壳、最小 phase（idle / turn / compaction / branch_summary / retry）、busy guard、subscriber、async listener settlement、turn snapshot 隔离、resources 快照、`abort()`、`steer()` / `followUp()` 内存队列、初始 queued message replay seed、queued message 写入 callback、`waitForIdle()`、`"all"` / `"one-at-a-time"` drain 模式，以及同一 loop 下一轮的 model / resources / system prompt / thinking level / active tools / tools 刷新；主 runtime 已接入 turn snapshot、provider stream hooks、可选 turn phase tracking、approval resume queue replay，以及下一次请求 drain 最新 completed run queue；session event 层已有 queued `steer` / `followUp` custom entry helper 与 `Agent.onQueuedMessage` 适配层；当前 chat route 已有 queued steering / follow-up UI + RPC 入口；**缺**自动触发 follow-up、save-point 刷新、stateful `Agent` 与 chat session / event store 生命周期接线 |
| `runAgentLoop` / `agentLoop`  | `packages/agent/src/agent-loop.ts`                                                  | [`agent-loop.ts`](../apps/desktop/src/main/agents/agent-loop.ts)；主 chat 仍用 AI SDK `streamText({ tools, stopWhen })` / 子 run 用 `generateText`                                                                                                                                                                      | **部分**：已落地独立 self-managed loop 的两轮上下文写回、parallel 完成顺序 vs source-order 写回、mixed sequential batch、通用 active tool allowlist、`beforeToolCall` 阻断 / 改参、`afterToolCall` patch / terminate、hook 错误 settlement、tool error continuation、sequential batch abort settlement、in-flight parallel tool abort settlement、model / tool abort signal、resources 快照、steering 注入、final turn follow-up、`prepareNextTurn` 替换 messages / model / resources / thinking level / active tools / tools，以及 async iterable / `ReadableStream` provider stream adapter 与 cancellation；主 runtime 已有持久化 queue 的 resume / next-request drain；**缺**接入主 `streamAgentChat` 流式路径和自动 follow-up 生命周期                                                                                                                                                                                                                          |
| Turn 快照 `createTurnState()` | `packages/agent/docs/agent-harness.md`                                              | [`agent-turn-state.ts`](../apps/desktop/src/main/agents/agent-turn-state.ts)                                                                                                                                                                                                                                            | **部分**：已落地每轮只读快照，冻结 messages / tools / stream options 浅拷贝，system prompt provider 只解析一次，provider credentials resolver 不进快照内容、保留请求期重取；`streamAgentChat()` 主 provider 请求已接入；stateful `Agent` 已通过 `prepareNextTurn` 刷新下一轮 model / system prompt / thinking level / tools；self-managed loop provider stream adapter 已有基础；**缺**主 `streamAgentChat()` 切换到自研 loop 与完整 settings setter                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `Session`（append-only 树）   | `packages/agent/src/harness/session/`                                               | [`agent-session-tree.ts`](../apps/desktop/src/main/agents/agent-session-tree.ts) + [`agent-session-events.ts`](../apps/desktop/src/main/agents/agent-session-events.ts) + [`chat_messages`](../apps/desktop/src/main/chat-messages.ts) 快照 + [`agent_events`](../apps/desktop/src/main/agents/agent-event-store.ts)    | **部分 + 双轨**：已有 in-memory append-only tree，覆盖 message / leaf / branch summary / compaction summary / custom message entry、leaf move、branch summary 注入、compaction context reset 和未知 entry 拒绝；prepared model messages、branch move、compaction summary、custom message、queued steering / follow-up 与 provider finish 后的 assistant / tool response messages 已写入 `agent_session_entry_appended`，approval resume 会从 event log 重建 provider context、replay pending queue 并只追加缺失消息；pending queued messages 可从 event log 计算，且 stateful `Agent` 的 queued write callback 已有 event-store writer 适配层；chat route 已有 queued steering / follow-up composer 入口，下一次请求会 drain 最新 completed run queue，UI 恢复仍靠 `UIMessage[]`。**缺**完整 branchable session 持久化、自动 follow-up 和 chat session 生命周期                                                                                                      |
| `ExecutionEnv`                | `packages/agent/src/harness/env/nodejs.ts`                                          | [`execution-env.ts`](../apps/desktop/src/main/agents/execution-env.ts) + [`tool-registry.ts`](../apps/desktop/src/main/agents/tool-registry.ts)                                                                                                                                                                         | **部分**：已有统一 cwd、abort、timeout、大输出 artifact、二进制输出清理、symlink 边界，补入 `findFiles` / `listDirectory` / `fileInfo` 这类 Pi `find` / `ls` / metadata 形态的只读工具，并提供基础 `fileSystem` Result API（含 temp / cleanup）与 `shell.exec()` Result 错误层；timeout / abort / spawn error 会保留 partial stdout / stderr；`readFile` / `editFile` / `writeFile` / `listDirectory` / `fileInfo` / `searchFiles` / `applyPatch` / `runCheck` / `rtkCommand` 已迁移到该边界；**缺**更完整的 self-managed agent loop 与 hook 能力                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Tool 钩子                     | `beforeToolCall` / `afterToolCall`（`packages/agent/src/types.ts`）                 | [`agent-loop.ts`](../apps/desktop/src/main/agents/agent-loop.ts)；主 runtime 仍是 `experimental_onToolCallStart` / `Finish`（仅写 event store）                                                                                                                                                                         | **部分**：独立 loop 已支持改参、阻断、patch result、terminate，hook 异常转 error result 且不 abort 同批其他工具；主 `streamAgentChat` 路径仍只有观测型 callback，尚不能在 provider loop 中改参或中止 batch                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
Pi `packages/agent/test/` 约 16 个专项文件；Etyon `apps/desktop/test/main/agents/` 当前 21 个文件。下表说明 Pi 用例形态与 Etyon 落地状态（截至文档更新时）。

| Pi 参考测试                            | 覆盖意图                                          | Etyon 对应测试                                                                                                                                                                                  | 状态                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| -------------------------------------- | ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `agent-loop.test.ts`                   | mock 两轮：tool 执行 → result 进下一轮；事件顺序  | `agent-loop.test.ts`                                                                                                                                                                            | **部分**（独立 loop 已覆盖两轮上下文写回与事件顺序；主 `streamAgentChat` 仍委托 `streamText`，未切到自研 loop）                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `agent-loop.test.ts`                   | parallel 完成顺序 vs tool result 写回顺序         | `agent-loop.test.ts`                                                                                                                                                                            | **已落地**（完成事件按完成顺序，写回模型上下文按 source order）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `agent-loop.test.ts`                   | steering 队列在 tool batch 完成后注入             | `agent-loop.test.ts`                                                                                                                                                                            | **部分**（独立 loop 已覆盖 batch 后注入、final turn follow-up 与 `prepareNextTurn`；尚无持久化 queue drain 模式和主 runtime 接线）                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `agent-loop.test.ts`                   | `beforeToolCall` 改参 / `afterToolCall` terminate | `agent-loop.test.ts`                                                                                                                                                                            | **部分**（独立 loop 已覆盖改参、阻断、patch output、terminate、hook error settlement 与 tool error continuation；主 `streamAgentChat` 仍无法在 AI SDK provider loop 中应用这些 hook）                                                                                                                                                                                                                                                                                                                                                                                   |
| `harness/agent-harness.test.ts`        | steer / follow-up / abort 队列、hook settlement   | `agent.test.ts`、`agent-loop.test.ts`、`agent-event-store.test.ts`、`agent-session-events.test.ts`、`agent-runtime.test.ts`、`agent-queue.test.ts`                                              | **部分**（已覆盖 `steer()` / `followUp()` 内存队列、initial queued messages replay、queue drain mode、queued message callback flush、queued session custom entry 持久化、`Agent.onQueuedMessage` 到 event-store writer 适配、pending queue 计算、chat runtime resume queue replay、下一次请求 drain 最新 completed run queue、chat route queued steering / follow-up UI 状态、`abort()` signal、busy guard、resources snapshot / next-turn refresh、turn snapshot 隔离与 subscriber error settlement；仍缺自动 follow-up、save point 与 harness-level hook settlement） |
| `agent.test.ts`                        | Agent state / subscribe / busy guard              | `agent.test.ts`、`agent-state.test.ts`、`agent-runtime.test.ts`                                                                                                                                 | **部分**（已覆盖 stateful Agent 构造、`prompt()` / `continue()`、mutators、phase guard、subscriber、async listener settlement、abort signal、队列、`waitForIdle()`、运行中 system prompt mutator 不污染当前 provider 且可刷新下一 loop turn，以及主 provider turn phase tracking；仍缺与 chat session 生命周期绑定）                                                                                                                                                                                                                                                    |
| `harness/types.test.ts`                | typed runtime error                               | `agent-errors.test.ts`、`agent-runtime.test.ts`                                                                                                                                                 | **部分**（已覆盖 `AgentRuntimeError` code / cause、hook failure 和 provider stream 创建失败；仍缺 session / tool 错误在主 runtime 的统一抛出边界）                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `harness/agent-harness-stream.test.ts` | stream options 与 provider hook                   | `agent-turn-state.test.ts`、`agent-stream-hooks.test.ts`、`agent-loop.test.ts`                                                                                                                  | **部分**（已覆盖 stream options 浅拷贝冻结、system prompt provider 单次解析、provider credentials resolver 请求期重取、request / payload / response hook patch 链；主 runtime 和子 agent 路径已接入；self-managed loop 已覆盖 provider stream adapter 与 cancellation；仍缺主 chat stream 接线）                                                                                                                                                                                                                                                                        |
| `harness/session.test.ts`              | branch、leaf、compaction 上下文重建               | `agent-session-tree.test.ts`、`agent-session-events.test.ts`、`approval-execution.test.ts`、`agent-event-store.test.ts`                                                                         | **部分**（已有 in-memory session tree，覆盖 message / leaf / branch summary / compaction summary / custom message context rebuild 与未知 entry 拒绝；session entry event 可持久化并重建 provider context，approval resume 可重建真实 provider context、replay pending queued steering 并只追加缺失消息；仍缺 JSONL / SQLite branchable session 生命周期）                                                                                                                                                                                                               |
| `harness/nodejs-env.test.ts`           | `ExecutionEnv` 边界                               | `execution-env.test.ts`、`tool-registry.test.ts`、`command-tools.test.ts`                                                                                                                       | **部分**（cwd、symlink、abort、timeout、输出 artifact、二进制输出清理、Result 风格路径 helper、exists、读写、分行/二进制读取、mkdir、remove、temp / cleanup、`fileInfo` / `listDir`、`shell.exec()` typed error、structured output event、timeout partial output、`findFiles`、`listDirectory`、`fileInfo`）                                                                                                                                                                                                                                                            |
...
| Mastra 形态 `tool-call-step`           | approval suspend → approve/deny                   | `approval-execution.test.ts`、`agent-runtime-harness.test.ts`、`agent-event-store.test.ts`、`index.test.ts`、`agent-recovery.test.ts`                                                           | **部分**（已有真实 provider step 触发 approval request、suspended / approve / deny、running 状态 pending resume、run-scoped approvalId、startup 最小恢复、resume provider context rebuild、pending queued steering replay、missing-only session append、assistant / tool response session 投影覆盖；failed 顶层 run 手动 retry 已有 RPC 与 chat route 入口；仍缺 tool-level / automatic retry UI）                                                                                                                                                                      |
| Mastra 形态 `subagent-tool`            | 子 agent 上下文隔离、无 metadata 泄漏             | `agent-runtime-harness.test.ts`、`apps/desktop/test/main/rpc/index.test.ts`、`apps/desktop/test/renderer/lib/chat/agent-run-trace.test.ts`                                                      | **部分**（runtime harness 已覆盖真实 child generate run、child request hooks、child context 隔离、受限 tools、child abort signal、child step budget、结构化 `subRunId`、summary transcript 清理；main RPC 已能按 run 读取 events / toolCalls；renderer tool trace 已能从 `subRunId` 懒加载 child trace，并能构建 run graph preview nodes / edges 与 display rows；child trace 面板可展示紧凑父子 run graph preview；仍缺完整 workbench 级 run graph UI）                                                                                                                |
...
1. mock 两轮 `LanguageModel`：已先在 `agent-loop.test.ts` 覆盖独立 loop，并补 `createAgentLoopStreamModel()` provider stream adapter / cancellation；后续仍需把 `streamAgentChat` 切到该 loop 后再断言真实 `agent_events` 序列。
2. 扩展 tool-level / approval retry UI 覆盖；failed 顶层 run 手动 retry 已有 RPC、chat route 和 helper 测试。
3. 子 agent：继续补完整 workbench 级 run graph、timeline 与组件级状态回归。
4. 继续扩展 `execution-env.ts` 的 Pi `nodejs-env.test.ts` 子集（FS Result 接口、错误标准化）。
5. 继续扩展已落地的 runtime harness：剩余手写 `streamText` mock 仅保留 provider creation failure 这类 faux provider 无法同步触发的低层边界。
6. 继续扩展 `agent-loop.test.ts`：queued steering / follow-up UI 和 RPC 已接线；后续补 runtime 生命周期与集成测试。
...
Etyon 首版已经具备 in-memory session tree，但仍未引入完整持久化 Session 树；event store 的 `agent_events` 仍应支持从 event log 重建部分 model context（用于 replay 和 harness-operator 诊断）。P5 引入持久化 branchable session log 时再完整迁移。
...
- 内建 tools：`readFile`、`searchFiles`、`findFiles`、`fileInfo`、`listDirectory`、`listProjectTree`、`gitDiff`、`memorySearch`、`rtkCommand`、`runCheck`、`applyPatch`、`editFile`、`writeFile`、`webSearch`。
...
- `ExecutionError`：`{ code: "timeout" | "aborted" | "spawn" | "shell_unavailable" | "unknown"; message: string; exitCode?: number; stdout?: string; stderr?: string }`
...
| ------------------ | ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | ---- | -------------------------------- |
| `general-purpose`  | 默认对话和轻量分析  | `fileInfo`、`findFiles`、`searchFiles`、`readFile`、`gitDiff`、`memorySearch`                                                                                  | 否   | 否   | 只读自动执行                     |
| `explore`          | 代码库探索和定位    | `listProjectTree`、`listDirectory`、`fileInfo`、`findFiles`、`searchFiles`、`readFile`、`memorySearch`                                                         | 否   | 否   | 只读自动执行                     |
| `plan`             | 方案设计和任务拆解  | `findFiles`、`fileInfo`、`searchFiles`、`readFile`、`gitDiff`、`memorySearch`、`agent_explore`                                                                 | 是   | 否   | 子 agent 只读                    |
| `coder`            | 小范围实现和修复    | `findFiles`、`fileInfo`、`searchFiles`、`listDirectory`、`readFile`、`gitDiff`、`memorySearch`、`applyPatch`、`editFile`、`writeFile`、`runCheck`、`webSearch` | 是   | 是   | 写入 / 长命令 / 网络工具需要审批 |
| `review`           | 代码审查和风险定位  | `gitDiff`、`listDirectory`、`fileInfo`、`findFiles`、`searchFiles`、`readFile`、`memorySearch`、`runCheck`                                                     | 可选 | 否   | read-only，check 可条件执行      |
...
- 不在 `general-purpose` / `explore` 默认只读 profile 启用 web / network tools；`coder` 的 `webSearch` 必须走 approval。
