...
- `setActiveTools(["read", "grep", "find", "ls", "stat", "questionnaire"])` — 仅暴露只读工具。
...
参考 `packages/agent/test/` 约 16 个专项文件；Etyon `apps/desktop/test/main/agents/` 当前 36 个 test 文件。下表说明参考用例形态与 Etyon 落地状态（截至文档更新时）。
...
| `harness/nodejs-env.test.ts`           | `ExecutionEnv` 边界                               | `execution-env.test.ts`、`tool-registry.test.ts`、`command-tools.test.ts`                                                                                                                                                                                                                           | **部分**（cwd、symlink、abort、timeout、输出 artifact、二进制输出清理、Result 风格路径 helper、exists、读写、分行/二进制读取、mkdir、remove、temp / cleanup、`fileInfo` / `listDir`、`shell.exec()` typed error、structured output event、timeout partial output、background process start / list / get / stop / cleanup、model-facing `bash background=true` / `processOutput` / `stopProcess` / `stat` / `mkdir` / `delete`、sandbox fail closed、`findFiles`、`listDirectory`、`fileInfo`）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
