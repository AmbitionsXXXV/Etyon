...
- [x] Chat panel 已通过 `useHotkey("Mod+Shift+Tab")` 切换 composer agent mode；切换结果显示在 prompt input 最左侧的 `Chat` / `Agent` segmented control，并在提交时通过 request body `agentMode` 临时覆盖本次 managed agent runtime 开关。裸 `Shift+Tab` 改为循环 permission mode（default → acceptEdits → bypass），chat 模式下禁用，permission 徽章带 pulse 反馈与 Kbd tooltip。
...
- Approval resume 的 projection 以最后一条 user message 作为合并边界；恢复后的 event-derived assistant suffix 会与恢复前已有的 assistant tool-call 气泡合并回同一条 assistant message，renderer 始终把一轮 assistant turn 渲染成单条连贯气泡，不再拆出第二条消息或显示续接标记。
...
- Chat viewport 会渲染 tool trace；当 AI SDK tool part 进入 `approval-requested` 状态时，可直接在 trace 行内批准或拒绝，并通过 AI SDK `originalMessages` / `start.messageId` continuation 机制继续同一条 assistant message，不在 approve 后新建 tool message、空 message 或文本 message；`git diff` / `git status` / `git log` / `git show` 这类安全只读 Git inspection 命令会直接通过 permission engine，且 `rtk git ...` 与 `runCheck` 路径也走同一只读 Git 判断。对 `bash` / `rtkCommand` / `runCheck` 这类仍需 approval 的命令，用户也可以选择批准并记住；runtime 会把 `projectPath + toolName + command + cwd` 写入 `settings.agents.approvals.commandAllowlist`，后续仅在同一 workspace、cwd、tool 且同属 bounded intent 时覆盖不同 argv，deny / risky 边界仍先执行。renderer 偏向 AI SDK 的 `UIMessage.parts`：一轮 assistant turn 渲染成单条连贯气泡，reasoning part 与 tool UI part 统一折叠进顶部一个 HeroUI Pro `ChainOfThought` 思考链（默认收起，流式中或存在 pending approval 时自动展开，approval 卡片始终保留可继续入口），最终回答正文在思考链下方以连贯 markdown 呈现；相邻重复的终态 structured tool part 会折叠为单张 trace 卡并显示重复次数。renderer 不再用正则解析文本流里的 `<antThinking>` / `Executed in ...` / `<function_calls><invoke ...>` transcript，也移除了 `settings.agents.showToolTraces` 开关，思考链恒定可用，只消费结构化 parts；请求提交后如果还没有第一段 assistant part，会先显示轻量 live 状态行，并能通过 `chat-request-phase` 展示 memory loading、model start 与 agent turn 状态。
- Settings 新增 `Agents` tab，用于控制 global enable、default profile、tool step budget、delegation 开关与 profile override 编辑，并提供只读 approval inbox 查看当前 pending tool approvals；tool trace 改为始终以思考链形态呈现，不再提供 `showToolTraces` 可见性开关；写入、网络与泛用本地命令审批由 permission engine 和 AI SDK approval message 强制执行，不再提供可关闭的 write approval 开关；命令 allowlist 是显式用户选择后的精确记忆规则，不会覆盖 deny 类边界或高风险命令检查；普通 chat stream 的直接 approve / deny / approve-and-remember 保留在 chat tool trace，run graph approval 的 approve / deny 已进入 chat Workbench，基础 run graph create / start / execute / run-until-idle / advance / retry 也已进入 chat Workbench，并已提供独立 `/agents/$sessionId` Workbench 页面用于离开 chat transcript 后继续查看 run graph / timeline / artifacts / approvals / diff。
...
- `buildChatStreamResponse()` 完成持久化时会用最新 user message 作为 projection 边界，而不是把完整 onFinish message list 当作原始 prefix；approval resume 场景下若请求前已有 assistant tool-call 气泡，投影后的 assistant suffix 会继承最新 stream metadata 并合并回同一条 assistant message，renderer timeline 把整轮 turn 渲染成单条连贯气泡，不再拆出第二条消息或显示续接标记。
