...
6. pending session writes（queue 中的 steer / follow-up 消息）已有 `agent_events` custom entry helper 和 pending replay 计算，后续接入 chat runtime 恢复。
...
| Tool 注册与过滤               | `coding-agent` tools + harness active tools                                         | [`profiles.ts`](../apps/desktop/src/main/agents/profiles.ts) + [`tool-registry.ts`](../apps/desktop/src/main/agents/tool-registry.ts)                                                                                                                                                                                   | **已对齐思路**：profile `toolPolicy` 决定暴露工具；readonly override 会移除写入 / 检查工具；子 agent scope 可去掉 approval / delegation tools                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 权限 / 审批                   | extension、`AgentSession` 层                                                        | [`permission-engine.ts`](../apps/desktop/src/main/agents/permission-engine.ts) + AI SDK `needsApproval`                                                                                                                                                                                                                 | Etyon **更显式**；UI 侧 `approval-requested` + `addToolApprovalResponse()`；Settings 已有只读 pending approval inbox；`approval-flow.test.ts` 覆盖 suspended profile resume 与 denied model error，`approval-execution.test.ts` 覆盖 approve 后真实本地工具执行；event-store 覆盖 running / suspended pending resume、run-scoped approvalId 与 provider context rebuild；startup 最小恢复会 fail 遗留 running run 并保留 suspended approval。**缺** retry UI                                                                                                                                                                                                                                                                                                                                                          |
| 子 agent                      | `coding-agent` extensions（非 agent 包核心测试）                                    | `agentExplore` / `agentPlan` / `agentReview` / `agentCoder` → child `generateText` run                                                                                                                                                                                                                                  | **agent-as-tool**：独立 `parentRunId`、摘要回父模型、普通 child 禁用 delegation / approval tools；approval-gated `agentCoder` 用父工具审批作为 execute handoff 边界，审批后 child `coder` 恢复完整 coder tool scope；测试已覆盖父 history 不进入 child messages，且 child summary 会清理内部 transcript 块；main RPC 已提供 `agents.inspectRun` 只读 trace 边界，renderer 可从 `subRunId` 懒加载 child trace，且已有 run graph preview nodes / edges 与 display rows 数据投影；child trace 面板可展示紧凑父子 run graph preview。**缺**完整 workbench 级 run graph UI                                                                                                                                                                                                                                                 |
| Provider stream 配置          | `packages/agent/test/harness/agent-harness-stream.test.ts`                          | [`agent-turn-state.ts`](../apps/desktop/src/main/agents/agent-turn-state.ts) + [`agent-stream-hooks.ts`](../apps/desktop/src/main/agents/agent-stream-hooks.ts)                                                                                                                                                         | **部分**：已覆盖 stream options / provider credentials 的 turn snapshot 边界、request headers / metadata patch、provider payload patch、response hook 与 hook typed error；主 `streamAgentChat()` provider stream 与 child `generateText()` 路径已接入 turn snapshot 和 request / payload / response hook；**缺**自研 loop provider stream adapter                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `harness/agent-harness.test.ts`        | steer / follow-up / abort 队列、hook settlement   | `agent.test.ts`、`agent-loop.test.ts`、`agent-event-store.test.ts`、`agent-session-events.test.ts`、`agent-runtime.test.ts`                                    | **部分**（已覆盖 `steer()` / `followUp()` 内存队列、initial queued messages replay、queue drain mode、queued message callback flush、queued session custom entry 持久化、pending queue 计算、chat runtime resume queue replay、`abort()` signal、busy guard、resources snapshot / next-turn refresh、turn snapshot 隔离与 subscriber error settlement；仍缺 chat runtime queued event 写入口、save point 与 harness-level hook settlement） |
...
| `harness/agent-harness-stream.test.ts` | stream options 与 provider hook                   | `agent-turn-state.test.ts`、`agent-stream-hooks.test.ts`                                                                                                       | **部分**（已覆盖 stream options 浅拷贝冻结、system prompt provider 单次解析、provider credentials resolver 请求期重取、request / payload / response hook patch 链；主 runtime 和子 agent 路径已接入；仍缺自研 loop provider stream adapter）                                                                                                                                                                                                |
| `harness/session.test.ts`              | branch、leaf、compaction 上下文重建               | `agent-session-tree.test.ts`、`agent-session-events.test.ts`、`agent-runtime.test.ts`、`agent-event-store.test.ts`                                             | **部分**（已有 in-memory session tree，覆盖 message / leaf / branch summary / compaction summary / custom message context rebuild 与未知 entry 拒绝；session entry event 可持久化并重建 provider context，approval resume 只追加缺失消息；仍缺 JSONL / SQLite branchable session 生命周期）                                                                                                                                                 |
...
| `harness/skills.test.ts`               | skill 加载与 system prompt 格式化                 | `skills.test.ts`、`packages/rpc/test/schemas/skills.test.ts`、`tool-manifest.test.ts`、`tool-policy.test.ts`、`tool-registry.test.ts`、`agent-runtime.test.ts` | **部分**（已覆盖 SKILL frontmatter / body、项目与全局 root、source 元数据、capabilities schema 默认值、XML prompt、model-invisible body 跳过、model-disabled reference 输出、内建 tool capability manifest 完整性、显式 skill capability 到内建工具集合的 policy 过滤，以及已选 skill capability 进入 chat runtime / tool registry 后收窄本次工具集合；仍缺 skill extension 自定义 tool 注册到 runtime 的执行绑定）                         |
...
| Mastra 形态 `tool-call-step`           | approval suspend → approve/deny                   | `approval-flow.test.ts`、`approval-execution.test.ts`、`agent-event-store.test.ts`、`agent-runtime.test.ts`                                                    | **部分**（已有 suspended / approve / deny、running 状态 pending resume、run-scoped approvalId、startup 最小恢复、resume provider context rebuild、assistant / tool response session 投影覆盖；仍缺 retry UI）                                                                                                                                                                                                                               |
| Mastra 形态 `subagent-tool`            | 子 agent 上下文隔离、无 metadata 泄漏             | `agent-runtime.test.ts`（delegation）、`apps/desktop/test/main/rpc/index.test.ts`、`apps/desktop/test/renderer/lib/chat/agent-run-trace.test.ts`               | **部分**（已覆盖 child context 隔离、受限 tools、结构化 `subRunId`、summary transcript 清理；main RPC 已能按 run 读取 events / toolCalls；renderer tool trace 已能从 `subRunId` 懒加载 child trace，并能构建 run graph preview nodes / edges 与 display rows；child trace 面板可展示紧凑父子 run graph preview；仍缺完整 workbench 级 run graph UI）                                                                                        |
...
| ------------------------------------------------ | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `createHarness(options?)`                        | `test/main/agents/agent-runtime-harness.ts` | **部分**：已有最小 project / chat session / faux model / stream 入口                                                                                               |
| `registerFauxProvider()` → `faux.setResponses()` | `test/main/agents/faux-provider.ts`         | **部分**：已有确定性 stream response 队列，并已接入 runtime harness                                                                                                |
| `harness.session.subscribe()` → `events[]`       | `test/main/agents/agent-runtime-harness.ts` | **部分**：`listEvents()` 可列出当前 session 持久化 agent events；`subscribeEvents()` 可 session-scoped live 捕获 in-process agent events；仍缺 Pi 风格 session API |
| `harness.faux.appendResponses()`                 | `test/main/agents/faux-provider.ts`         | **部分**：支持追加多轮 response，可通过 runtime harness 复用                                                                                                       |

**建议补测优先级**（对齐 Pi 收益最高、且不改架构即可做的项）：

1. mock 两轮 `LanguageModel`：已先在 `agent-loop.test.ts` 覆盖独立 loop；后续仍需把 `streamAgentChat` 切到该 loop 或补 provider stream adapter 后再断言真实 `agent_events` 序列。
2. 扩展 `approval-flow.test.ts`：retry UI。
...
5. 继续扩展已落地的 runtime harness：补齐 Pi 风格 session API，并替换更多手写 `streamText` mock。
6. 继续扩展 `agent-loop.test.ts`：provider stream adapter cancellation 回归；chat runtime queued event 写入口待接线后补集成测试。
...
| agent runtime      | mock model 第一次返回 tool call，第二次返回 final text；断言 tool result 进入下一步输入，事件顺序稳定                                                                                                    | `apps/desktop/test/main/agents/agent-loop.test.ts`、`apps/desktop/test/main/agents/agent-runtime.test.ts`                                                                                                                                  | `agent-loop.test.ts`                              | 部分（独立 `agent-loop.ts` 已覆盖两轮写回、事件顺序、parallel source-order 写回、mixed sequential batch、active tool allowlist、sequential abort settlement、in-flight parallel abort settlement、model / tool abort signal、resources 快照、steering、follow-up、`prepareNextTurn` 替换 messages / model / resources / thinking level / active tools / tools、`beforeToolCall` / `afterToolCall`、hook settlement、tool error continuation；主 `streamAgentChat` 仍委托 AI SDK `streamText`，未用该 loop 生成 UI stream / agent events） |
| session tree       | append-only message / leaf / branch summary / compaction summary / custom message entry 可重建 model context，并拒绝非法 branch target                                                                   | `apps/desktop/test/main/agents/agent-session-tree.test.ts`、`apps/desktop/test/main/agents/agent-session-events.test.ts`、`apps/desktop/test/main/agents/agent-runtime.test.ts`、`apps/desktop/test/main/agents/agent-event-store.test.ts` | `harness/session.test.ts`                         | 部分（已有 in-memory context rebuild，session entry event 可持久化并重建 provider context，approval resume 只追加缺失消息，assistant / tool response messages 已投影；缺完整 branchable session 生命周期接线）                                                                                                                                                                                                                                                                                                                            |
| turn state         | 每轮请求创建只读快照；运行中 settings / tools 变更不影响在途请求；system prompt provider 只执行一次；provider credentials 每请求重取                                                                     | `apps/desktop/test/main/agents/agent-turn-state.test.ts`、`apps/desktop/test/main/agents/agent-runtime.test.ts`                                                                                                                            | `harness/agent-harness-stream.test.ts`            | 部分（已覆盖 messages / tools / stream options 浅拷贝冻结、system prompt provider 单次解析、provider credentials resolver 不进入快照内容；主 `streamAgentChat()` provider request 已消费 turn snapshot；尚未接入自研 `agent-loop.ts`）                                                                                                                                                                                                                                                                                                    |
| stream hooks       | provider request / payload / response hook 可顺序 patch，并把 hook failure 包装为 typed runtime error                                                                                                    | `apps/desktop/test/main/agents/agent-stream-hooks.test.ts`、`apps/desktop/test/main/agents/agent-runtime.test.ts`                                                                                                                          | `harness/agent-harness-stream.test.ts`            | 部分（已覆盖 headers / metadata merge-delete、payload patch、response hook 与 `AgentRuntimeError("hook")`；主 `streamAgentChat()` provider stream 和子 agent `generateText()` 路径已接入 request / payload / response hook；仍缺自研 loop provider stream adapter）                                                                                                                                                                                                                                                                       |
| error handling     | 底层 Result 不抛异常；高层 runtime 抛 typed error 并保留 cause                                                                                                                                           | `apps/desktop/test/main/agents/agent-errors.test.ts`、`apps/desktop/test/main/agents/execution-env.test.ts`、`apps/desktop/test/main/agents/agent-runtime.test.ts`                                                                         | `harness/types.ts`                                | 部分（已覆盖共享 `AgentRuntimeError` code / cause、`ExecutionEnv` typed Result、主 provider request hook 失败时的 failed run settlement，以及 provider stream 创建失败的 typed reject；尚未把 session / tool runtime 错误统一迁移到高层 typed reject）                                                                                                                                                                                                                                                                                    |
| approval flow      | 需要审批时暂停，不执行工具；approve 后继续；deny 后给模型明确 tool error；approval state 可持久化                                                                                                        | `apps/desktop/test/main/agents/approval-flow.test.ts`、`apps/desktop/test/main/agents/approval-execution.test.ts`、`apps/desktop/test/main/agents/agent-event-store.test.ts`                                                               | Mastra `tool-call-step`                           | 部分（runtime 已覆盖 suspend、resume、跨 session 边界、历史 approval response 过滤、resume profile tool scope、approve 后真实本地工具执行、多 pending approval 等待全响应、running 状态 pending resume、run-scoped approvalId、startup 最小恢复、resume provider context rebuild 与模型可见 denied tool error；缺 retry UI）                                                                                                                                                                                                              |
