...
所有 Vite 配置文件使用原生 `resolve.tsconfigPaths: true` 同步 `tsconfig.json` 中的 `compilerOptions.paths`，无需额外引入 `vite-tsconfig-paths` 插件，也不需要手动维护 `resolve.alias`。
...
由于部分第三方依赖（如 `electron-store` 内部依赖的 `conf` → `ajv`）仍包含 CJS `require()` 调用，且 TypeScript 编译器等包内部使用了 `__filename` / `__dirname`，构建产物通过 `rollupOptions.output.banner` 注入 ESM shims：
...
主进程源码中需要 `createRequire` 时，应通过 `src/main/main-require.ts` 导出的 `mainRequire` 使用，避免与 banner 中的绑定名冲突。

这使得打包后的 ESM bundle 中残留的 `require()` / `__filename` / `__dirname` 调用能正常工作。
...
- **ESM shims（`require` / `__filename` / `__dirname`）**：第三方 CJS 依赖被 Rolldown 打包后保留的 `require()` / `__filename` / `__dirname` 调用需要此 shim；当所有依赖迁移到纯 ESM 后可移除
