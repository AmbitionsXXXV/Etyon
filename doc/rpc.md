...
- `memory.embeddingModels.list`：从本地模型目录实时返回默认远端 embedding model 与本地 embedding model catalog 的 available / missing / downloading 状态
- `memory.embeddingModels.install({ modelId })`：由 main process 下载本地 embedding model 文件，完成后返回最新 model status 列表
