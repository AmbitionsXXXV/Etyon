...
- Composer 的 `@` 指示器展示完整候选项，分组顺序为 skills、文件夹、文件；`$` 指示器专门用于筛选和选择 skills。两种入口里的 skill row 都使用紧凑单行列表展示 skill 名称、描述和来源项目，搜索触发通过 TanStack Pacer 做 180ms debounce，显式选择后会优先注入对应 skill instructions。
