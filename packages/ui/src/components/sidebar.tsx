...
import {
  ...
  SheetHeader,
  SheetTitle
} from "@etyon/ui/components/sheet"
...
function SidebarProvider({
  ...
  const sidebarOpen = openProp ?? _open
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      const nextOpenState =
        typeof value === "function" ? value(sidebarOpen) : value
      if (setOpenProp) {
      ...
      persistSidebarState(nextOpenState)
    },
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          "relative w-[calc(var(--sidebar-width)+0.375rem)] bg-transparent transition-[width] duration-300 ease-[cubic-bezier(0.25,1,0.5,1)]",
      ...
      >
        <div
          ...
          className={cn(
            "flex size-full flex-col bg-sidebar text-sidebar-foreground rounded-2xl",
            "shadow-[0_2px_8px_0_oklch(0_0_0/0.15),0_0_0_1px_oklch(1_0_0/0.04)_inset]",
            "group-data-[variant=floating]:rounded-2xl group-data-[variant=floating]:shadow-[0_2px_8px_0_oklch(0_0_0/0.15),0_0_0_1px_oklch(1_0_0/0.04)_inset]"
          )}
        >
          {children}
        </div>
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 flex w-4 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:inset-s-1/2 after:w-[2px] hover:after:bg-sidebar-border ltr:-translate-x-1/2 rtl:-translate-x-1/2",
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-lg p-2 text-left text-xs ring-sidebar-ring outline-hidden transition-[width,height,padding,background-color,color] duration-150 group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
  {
    variants: {
      variant: {
...
function SidebarMenuButton({
  ...
  const comp = useRender({
    ...
    render: tooltipContentProps ? <TooltipTrigger render={render} /> : render,
    state: {
      active: isActive,
      sidebar: "menu-button",
      size,
