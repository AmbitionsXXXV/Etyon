...
```tsx
// List queries
;["todos"][("todos", "filters", { status: "completed" })][ // All todos // Filtered todos
  ("todos", "search", "query")
][ // Search results
  // Detail queries (more specific = subset of list)
  ("todos", id)
][("todos", id, "comments")] // Single todo // Todo's comments
...
export function ErrorBoundary({ children }: Props) {
  return (
    <QueryErrorResetBoundary>
      {({ reset }) => (
        <ErrorBoundaryClass onReset={reset}>{children}</ErrorBoundaryClass>
      )}
    </QueryErrorResetBoundary>
  )
}
